...
function recuperaInsumo()
{
  ...
  if (document.formulario.unidadmedida.value == -1) {
	...
	ajax.open( "GET", "/prueba/jsp/CargarDevolucionInsumo.jsp?insumo="+valor1+"&cantidadinsumo="+valor2+"&unidadmedida="+valor3, true );
	ajax.send( "" );
...
function sacaInsumo()
{
	...
	var valor2 = document.formulario.cantidadinsumo.value;
	ajax.open( "GET", "/prueba/jsp/SacarDevolucionInsumo.jsp?cantidaddevolucion="+valor1+"&cantidadinsumo="+valor2, true );
	ajax.send( "" );
}
...
<form name = formulario action = "/prueba/servlet/GrabaDevolucionInsumo" onSubmit="return validar(this)" target="_self">
...
<button type="button" onClick="window.open('/prueba/jsp/UsoSistema.jsp?destino=ListaDevolucionInsumo.jsp&titulo=Lista Devolución de Insumo&filordenfabricacion=<%=filordenfabricacion%>&seleccion=<%=ordenfabricacion%>','_self')">Volver</button>
</form>
</body>
</html>