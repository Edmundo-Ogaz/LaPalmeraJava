...
<form name = formulario action = "/prueba/servlet/GrabaCliente" onSubmit="return validar(this)" target="_self">
...
<button type="button" onClick="window.open('/prueba/jsp/UsoSistema.jsp?destino=ListaCliente.jsp&titulo=Listado Cliente&filrut=<%=filrut%>&filnombre=<%=filnombre%>&fildireccion=<%=fildireccion%>&filcomuna=<%=filcomuna%>&filtelefono=<%=filtelefono%>&filfechanacimiento=<%=filfechanacimiento%>&filemail=<%=filemail%>&filpaginaweb=<%=filpaginaweb%>&filtipocliente=<%=filtipocliente%>&filobservacion=<%=filobservacion%>','_self')">Lista</button>
<button type="button" onClick="window.open('/prueba/jsp/UsoSistema.jsp?destino=FilCliente.jsp&titulo=Filtrado Cliente','_self')">Filtro</button>
