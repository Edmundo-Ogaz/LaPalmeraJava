...
function recuperaSubFamilia()
{
	...
	ajax.open( "GET", "/prueba/jsp/CargaSubFamilia.jsp?familiaSel="+document.formulario.familia.options[document.formulario.familia.selectedIndex].value, true );
	ajax.send( "" );
}
...
function muestraCodigoProducto()
{
	...
	ajax.open( "GET", "/prueba/jsp/CargaCodigoProducto.jsp?subfamiliaSel="+valor2+"&familiaSel="+valor1, true );	
...
<form name = formulario action = "/prueba/servlet/GrabaProducto" onSubmit="return validar(this)" target="_self">
...
<button type="button" onClick="window.open('/prueba/jsp/UsoSistema.jsp?destino=ListaProducto.jsp&titulo=Listado Producto&filcodigo=<%=filcodigo%>&filnombre=<%=filnombre%>&filtipoproducto=<%=filtipoproducto%>&filfamilia=<%=filfamilia%>&filsubfamilia=<%=filsubfamilia%>','_self')">Lista</button>
<button type="button" onClick="window.open('/prueba/jsp/UsoSistema.jsp?destino=FilProducto.jsp&titulo=Filtrado Producto','_self')">Filtro</button>
